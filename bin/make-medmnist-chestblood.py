...
from medmnist import PathMNIST, BloodMNIST, ChestMNIST
import numpy as np
...
if __name__ == '__main__':
    ...
    for split in ['train', 'test', 'val']:
        d1 = ChestMNIST(split=split, transform=tx, download=True)
        d2 = BloodMNIST(split=split, transform=tx, download=True)
    
    
        l1 = torch.tensor(d1.labels)
        li1 = torch.tensor(np.arange(len(d1.labels)))
        filter_two_class = (l1.sum(dim=1)==0).bool() + (l1[:,1]==1).bool()
        l1 = l1[filter_two_class]
        li1 = li1[filter_two_class]
        l1 = (l1.sum(dim=1) > 0).int()
        c1, ci1 = torch.sort(l1)
        l1,li1 = c1, li1[ci1]
        ...
        torch.save(idx1[shuffle], '../data/' + split + '-med-chest-idx.pt')
        torch.save(idx2[shuffle], '../data/' + split + '-med-bloodchest-idx.pt')
