...
class ClassifierChest(nn.Module):
    
    def __init__(self) -> None:
        super().__init__()
        print('loading weights')
        weights = torch.load('../medmnist_models/resnet18_28_1_chest.pth')
        self.network = ResNet18(3,14)
...
if __name__ == '__main__':
    ...
    def transform_chest_labels( targets):
        # targets = 1*(np.sum(targets, axis=-1) > 0)
        return targets
    

    # Train the Blood classifier
    train_set_b = ChestMNIST('train', transforms.ToTensor(), target_transform=transform_chest_labels)
    val_set_b = ChestMNIST('val', transforms.ToTensor(), target_transform=    transform_chest_labels)
    test_set_b = ChestMNIST('test', transforms.ToTensor(), target_transform=  transform_chest_labels)
    ...
    model_b = ClassifierChest().to(device)
    ...
    torch.save(model_b.state_dict(), '../experiments/classifiers_medmnist_cb/model_1.pt')
